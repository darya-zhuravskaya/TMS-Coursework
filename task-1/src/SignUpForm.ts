...
export default class SignUpForm {
  ...
  private validateEmail() {
    ...
    const [userName, host] = this.email.split("@");
    if (!userName || !host) {
  ...
  private validatePassword() {
    ...
    if (passwordFormats.some((format) => !this.password.match(format))) {
      throw new Error(
        "Password should contain at least one capital character, one lower character, one digit, one special symbol",
      );
    }
  }
  ...
  private validateName(name: string, errorName: string) {
    ...
    if (name.length < nameMinLength) {
      throw new Error(
        `${errorName} should contain at least ${nameMinLength} characters`,
      );
    }
